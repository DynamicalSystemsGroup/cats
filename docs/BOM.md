...
Full contract: **[`BomRegistry.md`](BomRegistry.md)** — append-only query index (`cats/network/registry/`), not the envelope store (`BomLdpStore` / Solid) and not LDN. `Runtime.execute` writes after LDP/Solid locators are known (fail closed). `POST /cat/node/init` accepts `order_cid` (bootstrap), or `bom_cid` / unique `data_cid` via the index (`GET /ldp/registry/…`). Remaining gaps: mesh federation, `content_id`→locators (CAS-over-HTTP), Solid dual-write of registry records.
