...
- `function_cid` resolves to `{process_cid, infrafunction_cid}` - Function (FaaS) is CID-ed as the pairing of Process [REPL(aC)] (the Read-Eval-Print Loop as Code that composes and submits transport callables plus a Transfer Higher-Order Function [tHOF] — the input→output data transform) and the InfraFunction (FaaS) actuator that dispatches that tHOF onto the Plant.
