...
**Content-Addressable Transformers (CATs)** is an edge-computing framework that establishs a **[Data Mesh](https://www.datamesh-architecture.com/#what-is-data-mesh)** as a *self-service* Platform for *intra/inter-orginizational Data Service Collaboration* with *verifiable* **Data Provenance**. **CAT Nodes** are edges/mesh-peers intended to be system *integration points* that enable **[Data Product](https://www.datamesh-architecture.com/#data-product)** collaboration on the *verification, retrieval, and re-execution* of **interoperable** data processing workloads and their components *intra/inter-orginizationally* via *content-addressing* **[Bills-Of-Materials (BOMs)](https://en.wikipedia.org/wiki/Bill_of_materials)** and their contents as a means of Data Provenance. BOMs are used to **verify** the means of processing as code (input, transformation / process, output, & infrastructure) beteen CAT Nodes. The maintenance and reporting of [data and process lineage & provenance](https://bi-insider.com/posts/data-lineage-and-data-provenance/) using BOMs as *supply-chains of evidence* certifies the accuracy of services offered by *Data Products*. 
...
### How CATs' *Architectural Quantum* sustains *Data Initiatives* & *Product Collaboration* on a Data Mesh's Architectural Planes:
